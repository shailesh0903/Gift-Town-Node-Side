...
const Products = db.Products; // Assuming Categories model is exported from the models module
...
module.exports = async (req, res, next) => {
    try {
        ...
        const searchResults = await Products.find({
            $or: [
                { product_name: { $regex: query, $options: 'i' } }, // Case-insensitive search by category name
                // Case-insensitive search by tag
            ]
        });
